...
def elastic_deformation(batch, alpha=1.0, sigma=10.0, kSize=17, auto_kSize=True, order=0, fill_mode="mirror", fill_value=0.0):
    """
    Elastic deformation (P Y Simard, D Steinkraus and J C Platt; 2003).
    Available at: https://ieeexplore.ieee.org/document/1227801

    batch - Batch of 4D tensors
    alpha - Displacement scaling factor
    sigma - Smoothing factor (gaussian kernel std dev)
    kSize - Kernel size
    auto_kSize - Autocompute kernel size given the sigma
    order - Coordinate mapping interpolation (see tf.keras.ops.image.map_coordinates documentation)
    fill_mode - Boundary fill mode
    fill_value - Fill value if fill_mode="constant"
    """
...
def grid_deformation(batch, distort_limits=(-0.3, 0.3), grid_size=5, order=0, fill_mode="mirror", fill_value=0.0):
    """
    Tensorflow implementation of the GridDistortion algorithm 
    from Albumentations (Alexander B, V Iglovikov, E Khvedchenya, A Parinov, M Druzhinin, A Kalinin; 2020).
    Available at: github.com/albumentations-team/albumentations/blob/66212d77/albumentations/augmentations/geometric/distortion.py#L872

    batch - Batch of 4D tensors
    distort_limits - Limits of the distortion coefficient X~U(a,b)
    grid_size - Number of grid cells of the distorted image
    order - Coordinate mapping interpolation (see tf.keras.ops.image.map_coordinates documentation)
    fill_mode - Boundary fill mode
    fill_value - Fill value if fill_mode="constant"
    """
    ...
    for i in tf.range(1 + grid_size):
        # Select a row of cells x0 and a column of cells y0
        begin_x = x
        begin_y = y
        ...
        # Generate the grid deformations for a row x0 and a column y0. Each deformation is a sum of previous distortions, smoothing them out
    ...
    dx = dx.concat()
    dy = dy.concat()
    dx, dy = tf.meshgrid(dx, dy) # Generate distortion grid
    output = tf.TensorArray(batch.dtype, size=batch_shape[0])
    # Iterate batch
    for i in tf.range(batch_shape[0]):
        output_i = tf.TensorArray(batch.dtype, size=batch_shape[3])
